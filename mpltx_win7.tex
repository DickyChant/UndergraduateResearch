...
贝叶斯神经网络是基于贝叶斯理论的人工神经网络结构，在面对小样本学习时有独特的优势。\par
贝叶斯公式有着如下表述：
\begin{equation}
    p(W | X, Y)=\frac{p(W) p(Y | X, W)}{p(Y | X)}
\end{equation}
式中，$W$代表网络中的参数，$(X,Y)$则是数据集数据，$X$代表输入，而$Y$代表输出。\par
因此$p(Y|X)$是给定的，而剩下的各项，$p(W)$是参数的先验分布，而$p(Y|X,W)$则代表了网络在给定参数$W$和输入$X$时获得的输出概率。\par
...
\begin{enumerate}
    ...
    \item $\log p(Y | X, W)$\par 这一项的意义是给定参数$W$与输入$X$下，输出的分布，由于我们取$W$各分量均为高斯分布并认为可以足够接近真实分布，因此
    可以认为，网络输出也应该是一个高斯分布，其期望值正是$X$对应的$Y_{pred}$，所以$p(Y | X, W)=\frac{1}{\sqrt{2 \pi} \sigma} \exp \left(\left(Y-Y_{p r e d}\right)^{T} \sigma_{-1}\left(Y-Y_{p r e d}\right)\right)$，
...
通过不同的参数选择，TRENTO可以给出近似各种模型的初态分布，通过TRENTO参数的确定，我们或许可以选择最为接近实际的初态模型；而通过VISHNU参数的确定，我们可以直接确定QGP自身的宏观物理性质。\par
    ...
    \begin{enumerate}
        \item 对于初态模型，我们将TRENTO所用到的所有参数都作为输入，包括有总规模因子$\mathrm{N}$、约化密度参数$p$、权重因子抽样符合的gamma分布参数$k$以及粒子分布与能量密度转换参数$\omega$；
        ...
        模型将通过调制整体归一化系数完成控制。
        \item 对于末态粒子观测量，在相对论性重离子碰撞中，粒子的产量$dN/d\eta$以及集体流系数$v_n$，是描述碰撞事件末态的重要指标，因此我们选择使用这些观测量来进行系数的拟合，同时，将所选择的观测量按照中心度进行离散化。
    \end{enumerate}
    综上，数据的选取如表所示。
    \item 神经网络结构的选取：\par
    由于相对论性重离子碰撞的模拟十分消耗计算资源，而为了压低涨落，所需要的模拟事件数目更多，因此，只拥有250组数据供训练，50+1组数据供测试。所以，在本课题中采用贝叶斯神经网络进行拟合，网络的结构如图。    
\end{enumerate}
\subsubsection{结果与分析}
对于
...
\begin{enumerate}
    ...
    \par
    处理积分流时（在$\eta$方向进行积分，而前向后向的分布应当是完全相关的）贝叶斯展开在处理低维数据中有着很好的表现，可以同时完成上述功能。\par
    ...
    \begin{enumerate}
        ...
        实验上对于解关联的测定，常常是通过前向后向$\eta$绝对值相同区间来实现。
    \end{enumerate}
